...
describe("hasMany extra properties", function() {
	...
	describe("if passed to addAccessor", function () {
		...
		it("should be added to association", function (done) {
          ...
          foo.bar = 0;
          foo.baz = "test";
					people[0].addPets(pets, { someData : foo }, function (err) {
						...
						Person.find({ name: "John" }, { autoFetch : true }).first(function (err, John) {
							...
							John.pets[0].extra.should.have.property("someData");
							John.pets[0].extra.someData.should.have.property("bar");
							John.pets[0].extra.someData.should.have.property("baz");
							should(John.pets[0].extra.someData instanceof Object);
